...
void convolution_layer(cl_command_queue queue, cl_kernel kernel, cl_mem* inputs, cl_mem* outputs, cl_mem* weights, cl_mem* biases,
    int input_dim, int output_dim, int nbyn, int batch_size, cl_mem *bit_check1, cl_mem *bit_check2)
{
    cl_int err;
    ...
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel, 8, sizeof(cl_mem), bit_check1);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel, 9, sizeof(cl_mem), bit_check2);
    CHECK_ERROR(err);
...
void cnn(float* images, float* network, int* labels, float* confidences, int num_of_image) {
    ...
    for (int i = 0; i < 21; ++i) {
        if (i == 0 || i == 1 || i == 3 || i == 4 || i == 6 || i == 7 || i == 8 || i == 10 || i == 11 || i == 12 || i == 14 || i == 15) { // 컨볼루션 레이어가 있는 층
            size_t buffer_size = batch_size * OUTPUT_DIM[i] * NBYN[i] * NBYN[i];

            bit_check[k] = clCreateBuffer(context, CL_MEM_READ_WRITE, buffer_size, NULL, &err);
            CHECK_ERROR(err);

            // 호스트 메모리를 0으로 초기화하고, GPU 버퍼에 쓰기
            char* host_buffer = (char*)calloc(buffer_size, sizeof(char));

            err = clEnqueueWriteBuffer(queue, bit_check[k], CL_TRUE, 0, buffer_size, host_buffer, 0, NULL, NULL);
            CHECK_ERROR(err);
            // printf("%d %d\n", k, i);
    ...
    for (int i = 0; i < num_of_image; i += batch_size) {
        ...
        convolution_layer(queue, conv_kernel, &imageBuf, &layerBuf[0], &wBuf[0], &bBuf[0], INPUT_DIM[0], OUTPUT_DIM[0], NBYN[0], current_batch_size, NULL, &bit_check[0]);
        convolution_layer(queue, conv_kernel, &layerBuf[0], &layerBuf[1], &wBuf[1], &bBuf[1], INPUT_DIM[1], OUTPUT_DIM[1], NBYN[1], current_batch_size, &bit_check[0], &bit_check[1]);
        max_pooling_layer(queue, pooling_kernel, &layerBuf[1], &layerBuf[2], INPUT_DIM[2], NBYN[2], current_batch_size);

        convolution_layer(queue, conv_kernel, &layerBuf[2], &layerBuf[3], &wBuf[3], &bBuf[3], INPUT_DIM[3], OUTPUT_DIM[3], NBYN[3], current_batch_size, &bit_check[1], &bit_check[2]);
        convolution_layer(queue, conv_kernel, &layerBuf[3], &layerBuf[4], &wBuf[4], &bBuf[4], INPUT_DIM[4], OUTPUT_DIM[4], NBYN[4], current_batch_size, &bit_check[2], &bit_check[3]);
        max_pooling_layer(queue, pooling_kernel, &layerBuf[4], &layerBuf[5], INPUT_DIM[5], NBYN[5], current_batch_size);

        convolution_layer(queue, conv_kernel, &layerBuf[5], &layerBuf[6], &wBuf[6], &bBuf[6], INPUT_DIM[6], OUTPUT_DIM[6], NBYN[6], current_batch_size, &bit_check[3], &bit_check[4]);
        convolution_layer(queue, conv_kernel, &layerBuf[6], &layerBuf[7], &wBuf[7], &bBuf[7], INPUT_DIM[7], OUTPUT_DIM[7], NBYN[7], current_batch_size, &bit_check[4], &bit_check[5]);
        convolution_layer(queue, conv_kernel, &layerBuf[7], &layerBuf[8], &wBuf[8], &bBuf[8], INPUT_DIM[8], OUTPUT_DIM[8], NBYN[8], current_batch_size, &bit_check[5], &bit_check[6]);
        max_pooling_layer(queue, pooling_kernel, &layerBuf[8], &layerBuf[9], INPUT_DIM[9], NBYN[9], current_batch_size);

        convolution_layer(queue, conv_kernel, &layerBuf[9], &layerBuf[10], &wBuf[10], &bBuf[10], INPUT_DIM[10], OUTPUT_DIM[10], NBYN[10], current_batch_size, &bit_check[6], &bit_check[7]);
        convolution_layer(queue, conv_kernel, &layerBuf[10], &layerBuf[11], &wBuf[11], &bBuf[11], INPUT_DIM[11], OUTPUT_DIM[11], NBYN[11], current_batch_size, &bit_check[7], &bit_check[8]);
        convolution_layer(queue, conv_kernel, &layerBuf[11], &layerBuf[12], &wBuf[12], &bBuf[12], INPUT_DIM[12], OUTPUT_DIM[12], NBYN[12], current_batch_size, &bit_check[8], &bit_check[9]);
        max_pooling_layer(queue, pooling_kernel, &layerBuf[12], &layerBuf[13], INPUT_DIM[13], NBYN[13], current_batch_size);

        convolution_layer(queue, conv_kernel, &layerBuf[13], &layerBuf[14], &wBuf[14], &bBuf[14], INPUT_DIM[14], OUTPUT_DIM[14], NBYN[14], current_batch_size, &bit_check[9], &bit_check[10]);
        convolution_layer(queue, conv_kernel, &layerBuf[14], &layerBuf[15], &wBuf[15], &bBuf[15], INPUT_DIM[15], OUTPUT_DIM[15], NBYN[15], current_batch_size, &bit_check[10], &bit_check[11]);
        convolution_layer(queue, conv_kernel, &layerBuf[15], &layerBuf[16], &wBuf[16], &bBuf[16], INPUT_DIM[16], OUTPUT_DIM[16], NBYN[16], current_batch_size, &bit_check[11], NULL);
