...
void convolution_layer(cl_command_queue queue, cl_kernel conv_mul_kernel, cl_kernel conv_sum_kernel,
	cl_mem* inputs, cl_mem* outputs, cl_mem* temp, cl_mem* weights, cl_mem* biases,
	int input_dim, int output_dim, int nbyn, int batch_size)
{
	cl_int err;

	err = clSetKernelArg(conv_mul_kernel, 0, sizeof(cl_mem), inputs);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_mul_kernel, 1, sizeof(cl_mem), temp);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_mul_kernel, 2, sizeof(cl_mem), weights);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_mul_kernel, 3, sizeof(int), &input_dim);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_mul_kernel, 4, sizeof(int), &output_dim);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_mul_kernel, 5, sizeof(int), &nbyn);
	CHECK_ERROR(err);

	size_t global_work_size1[2] = { nbyn * nbyn * output_dim * input_dim, batch_size };
	err = clEnqueueNDRangeKernel(queue, conv_mul_kernel, 2, NULL, global_work_size1, NULL, 0, NULL, NULL);
	CHECK_ERROR(err);

	err = clSetKernelArg(conv_sum_kernel, 0, sizeof(cl_mem), temp);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_sum_kernel, 1, sizeof(cl_mem), outputs);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_sum_kernel, 2, sizeof(cl_mem), biases);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_sum_kernel, 3, sizeof(int), &input_dim);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_sum_kernel, 4, sizeof(int), &output_dim);
	CHECK_ERROR(err);
	err = clSetKernelArg(conv_sum_kernel, 5, sizeof(int), &nbyn);
	CHECK_ERROR(err);

	size_t global_work_size2[2] = { nbyn * nbyn * output_dim, batch_size };
	err = clEnqueueNDRangeKernel(queue, conv_sum_kernel, 2, NULL, global_work_size2, NULL, 0, NULL, NULL);
	CHECK_ERROR(err);
}
...
void cnn(float* images, float* network, int* labels, float* confidences, int num_of_image) {
	...
	cl_kernel conv_mul_kernel = clCreateKernel(program, "conv_mul_kernel", &err);
	CHECK_ERROR(err);
	cl_kernel conv_sum_kernel = clCreateKernel(program, "conv_sum_kernel", &err);
	CHECK_ERROR(err);
	cl_kernel conv_sum1_kernel = clCreateKernel(program, "conv_sum1_kernel", &err);
	CHECK_ERROR(err);
	...
	cl_mem tempBuf = clCreateBuffer(context, CL_MEM_READ_WRITE, batch_size * sizeof(float) * 64 * 64 * 1024, NULL, &err);
	CHECK_ERROR(err);
	...
	for (int i = 0; i < num_of_image; i += batch_size) {
		...
		convolution_layer(queue, conv_mul_kernel, conv_sum1_kernel, &imageBuf, &layerBuf[0], &tempBuf, &wBuf[0], &bBuf[0], INPUT_DIM[0], OUTPUT_DIM[0], NBYN[0], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[0], &layerBuf[1], &tempBuf, &wBuf[1], &bBuf[1], INPUT_DIM[1], OUTPUT_DIM[1], NBYN[1], current_batch_size);
		max_pooling_layer(queue, pooling_kernel, &layerBuf[1], &layerBuf[2], INPUT_DIM[2], NBYN[2], current_batch_size);

		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[2], &layerBuf[3], &tempBuf, &wBuf[3], &bBuf[3], INPUT_DIM[3], OUTPUT_DIM[3], NBYN[3], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[3], &layerBuf[4], &tempBuf, &wBuf[4], &bBuf[4], INPUT_DIM[4], OUTPUT_DIM[4], NBYN[4], current_batch_size);
		max_pooling_layer(queue, pooling_kernel, &layerBuf[4], &layerBuf[5], INPUT_DIM[5], NBYN[5], current_batch_size);

		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[5], &layerBuf[6], &tempBuf, &wBuf[6], &bBuf[6], INPUT_DIM[6], OUTPUT_DIM[6], NBYN[6], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[6], &layerBuf[7], &tempBuf, &wBuf[7], &bBuf[7], INPUT_DIM[7], OUTPUT_DIM[7], NBYN[7], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[7], &layerBuf[8], &tempBuf, &wBuf[8], &bBuf[8], INPUT_DIM[8], OUTPUT_DIM[8], NBYN[8], current_batch_size);
		max_pooling_layer(queue, pooling_kernel, &layerBuf[8], &layerBuf[9], INPUT_DIM[9], NBYN[9], current_batch_size);

		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[9], &layerBuf[10], &tempBuf, &wBuf[10], &bBuf[10], INPUT_DIM[10], OUTPUT_DIM[10], NBYN[10], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[10], &layerBuf[11], &tempBuf, &wBuf[11], &bBuf[11], INPUT_DIM[11], OUTPUT_DIM[11], NBYN[11], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[11], &layerBuf[12], &tempBuf, &wBuf[12], &bBuf[12], INPUT_DIM[12], OUTPUT_DIM[12], NBYN[12], current_batch_size);
		max_pooling_layer(queue, pooling_kernel, &layerBuf[12], &layerBuf[13], INPUT_DIM[13], NBYN[13], current_batch_size);

		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[13], &layerBuf[14], &tempBuf, &wBuf[14], &bBuf[14], INPUT_DIM[14], OUTPUT_DIM[14], NBYN[14], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[14], &layerBuf[15], &tempBuf, &wBuf[15], &bBuf[15], INPUT_DIM[15], OUTPUT_DIM[15], NBYN[15], current_batch_size);
		convolution_layer(queue, conv_mul_kernel, conv_sum_kernel, &layerBuf[15], &layerBuf[16], &tempBuf, &wBuf[16], &bBuf[16], INPUT_DIM[16], OUTPUT_DIM[16], NBYN[16], current_batch_size);
	...
	err = clReleaseMemObject(tempBuf);
	CHECK_ERROR(err);
	for (int i = 0; i < 17; ++i) {
		if (i == 2 || i == 5 || i == 9 || i == 13) i++;    // pooling layer has no weights and biases
