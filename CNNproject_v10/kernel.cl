...
__kernel void pooling_kernel(
    ...
    outputs[(batchIdx * outDim + outNeuron) * nbyn * nbyn + spatialIdx] = maxVal;
}
...
__kernel void conv1_kernel(
    ...
    const int l_row = l_spatialIdx / 8;
    const int l_col = l_spatialIdx % 8;
...
__kernel void conv3_kernel(
    ...
    const int inputSize = nbyn * nbyn;
    const int outNeuron = flatIdx / inputSize;
    const int spatialIdx = flatIdx % inputSize;
    const int row = spatialIdx / nbyn;
    const int col = spatialIdx % nbyn;

    const int l_flatIdx = get_local_id(0);
    const int l_inNeuron = l_flatIdx / inputSize;
    const int l_spatialIdx = l_flatIdx % inputSize;
    const int l_channel = 64 / inputSize;
    ...
    for (inNeuron = 0; inNeuron < inDim; inNeuron += l_channel) {
        inputOffset = (batchIdx * inDim + inNeuron) * inputSize;
        filterOffset = (outNeuron * inDim + inNeuron) * 9;

        l_inputs[l_inNeuron * inputSize + l_spatialIdx] = inputs[inputOffset + l_flatIdx];
        ...
        for (int ch = 0; ch < l_channel; ++ch) {
            for (fRow = 0; fRow < 3; ++fRow) {
                for (fCol = 0; fCol < 3; ++fCol) {
                    ...
                    y = row + fRow - 1;
                    if (x >= 0 && x < nbyn && y >= 0 && y < nbyn) {
                        tmp = l_inputs[ch * inputSize + y * nbyn + x];
                        if(tmp != 0) {
