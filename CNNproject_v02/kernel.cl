...
__kernel void conv_lm1_kernel ( 
	...
	for (inNeuron = 0; inNeuron < inDim; ++inNeuron) {
		...
		if (l_row == 0 && l_col == 0) {
			for (int i = 0; i < 9; ++i) {
				l_filter[i] = filter[filterOffset + i];
			}
		}
		barrier(CLK_LOCAL_MEM_FENCE);
		...
		for (fRow = 0; fRow < 3; ++fRow) {
			for (fCol = 0; fCol < 3; ++fCol) {
				...
				y = row + fRow - 1;
				if (x >= 0 && x < nbyn && y >= 0 && y < nbyn) {
					sum += inputs[inputOffset + nbyn * y + x] * l_filter[3 * fRow + fCol];
				}
			}
		}
	}
...
__kernel void conv_lm2_kernel ( 
	...
	float sum = 0.0f;
	for (inNeuron = 0; inNeuron < inDim; ++inNeuron) {
		inputOffset = inNeuron * offset + row * nbyn + col;
		filterOffset = (outNeuron * inDim + inNeuron) * 9;
		
		barrier(CLK_LOCAL_MEM_FENCE);
		l_input[l_row * nbyn + l_col] = inputs[inputOffset];
		barrier(CLK_LOCAL_MEM_FENCE);
		...
		for (fRow = 0; fRow < 3; ++fRow) {
			for (fCol = 0; fCol < 3; ++fCol) {
				...
				y = l_row + fRow - 1;
				if (x >= 0 && x < nbyn && y >= 0 && y < nbyn) {
					sum += l_input[nbyn * y + x] * filter[filterOffset + 3 * fRow + fCol];
				}
			}
		}
	}
	...
	for (inNeuron = 0; inNeuron < inDim; ++inNeuron) {
		...
		for (fRow = 0; fRow < 3; ++fRow) {
			for (fCol = 0; fCol < 3; ++fCol) {
				...
				y = l_row + fRow - 1;
				if (x >= 0 && x < nbyn && y >= 0 && y < nbyn) {
					sum += l_input[nbyn * y + x] * l_filter[3 * fRow + fCol];
				}
			}
		}
	}
