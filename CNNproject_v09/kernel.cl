...
__kernel void conv_tile_kernel(
    ...
    const int spatial_idx_start = get_global_id(0) * 2;
    ...
    for (i = 0; i < inDim; i += wgSize) {
        ...
        for (int s = 0; s < 2; ++s) {
            const int spatial_idx = spatial_idx_start + s;
            const int row = spatial_idx / nbyn;
            const int col = spatial_idx % nbyn;
            ...
            for (j = 0; j < wgSize; ++j) {
                ...
                for (k = 0; k < 9; ++k) {
                    ...
                    if (tmp != 0) {
                        if (s == 0) {
                            sum1 += tmp * filter[filterIdx + k];
                        } else {
                            sum2 += tmp * filter[filterIdx + k];
                        }
                    }
                }
            }
    ...
    outputs[batch_idx * outDim * nbyn * nbyn + outNeuron * nbyn * nbyn + spatial_idx_start] = fmax(sum1 + biases[outNeuron], 0.0f);
    outputs[batch_idx * outDim * nbyn * nbyn + outNeuron * nbyn * nbyn + spatial_idx_start + 1] = fmax(sum2 + biases[outNeuron], 0.0f);
}
