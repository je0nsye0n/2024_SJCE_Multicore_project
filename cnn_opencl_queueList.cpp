...
void cnn_init(cl_context* context, cl_command_queue* queue, cl_program* program) {
    ...
    *queue = clCreateCommandQueueWithProperties(*context, device, 0, &err);
    ...
    for (int i = 0; i < 23; i++) {
        queuelist[i] = clCreateCommandQueueWithProperties(*context, device, 0, &err);
    }
...
void cnn(float* images, float* network, int* labels, float* confidences, int num_of_image) {
    ...
    for (int i = 0; i < num_of_image; i += batch_size) {
        ...
        if (i == 0) {
            ...
            err = clEnqueueWriteBuffer(queuelist[0], imageBuf, CL_FALSE, 0,
                sizeof(float) * 3 * 32 * 32 * current_batch_size,
                images + i * 32 * 32 * 3, 0, NULL, &write_event);
            CHECK_ERROR(err);

            convolution_layer(queuelist[1], conv_kernel, &imageBuf, &layerBuf[0], &wBuf[0], &bBuf[0],
                INPUT_DIM[0], OUTPUT_DIM[0], NBYN[0], current_batch_size,
                &write_event, &conv_event[0]);

            convolution_layer(queuelist[2], conv_kernel, &layerBuf[0], &layerBuf[1], &wBuf[1], &bBuf[1],
                INPUT_DIM[1], OUTPUT_DIM[1], NBYN[1], current_batch_size,
                &conv_event[0], &conv_event[1]);

            max_pooling_layer(queuelist[3], pooling_kernel, &layerBuf[1], &layerBuf[2],
                INPUT_DIM[2], NBYN[2], current_batch_size,
                &conv_event[1], &pool_event[0]);



            convolution_layer(queuelist[4], conv_kernel, &layerBuf[2], &layerBuf[3], &wBuf[3], &bBuf[3],
                INPUT_DIM[3], OUTPUT_DIM[3], NBYN[3], current_batch_size,
                &pool_event[0], &conv_event[2]);

            convolution_layer(queuelist[5], conv_kernel, &layerBuf[3], &layerBuf[4], &wBuf[4], &bBuf[4],
                INPUT_DIM[4], OUTPUT_DIM[4], NBYN[4], current_batch_size,
                &conv_event[2], &conv_event[3]);

            max_pooling_layer(queuelist[6], pooling_kernel, &layerBuf[4], &layerBuf[5],
                INPUT_DIM[5], NBYN[5], current_batch_size,
                &conv_event[3], &pool_event[1]);



            convolution_layer(queuelist[7], conv_kernel, &layerBuf[5], &layerBuf[6], &wBuf[6], &bBuf[6],
                INPUT_DIM[6], OUTPUT_DIM[6], NBYN[6], current_batch_size,
                &pool_event[1], &conv_event[4]);

            convolution_layer(queuelist[8], conv_kernel, &layerBuf[6], &layerBuf[7], &wBuf[7], &bBuf[7],
                INPUT_DIM[7], OUTPUT_DIM[7], NBYN[7], current_batch_size,
                &conv_event[4], &conv_event[5]);

            convolution_layer(queuelist[9], conv_kernel, &layerBuf[7], &layerBuf[8], &wBuf[8], &bBuf[8],
                INPUT_DIM[8], OUTPUT_DIM[8], NBYN[8], current_batch_size,
                &conv_event[5], &conv_event[6]);

            max_pooling_layer(queuelist[10], pooling_kernel, &layerBuf[8], &layerBuf[9],
                INPUT_DIM[9], NBYN[9], current_batch_size,
                &conv_event[6], &pool_event[2]);



            convolution_layer(queuelist[11], conv_kernel, &layerBuf[9], &layerBuf[10], &wBuf[10], &bBuf[10],
                INPUT_DIM[10], OUTPUT_DIM[10], NBYN[10], current_batch_size,
                &pool_event[2], &conv_event[7]);

            convolution_layer(queuelist[12], conv_kernel, &layerBuf[10], &layerBuf[11], &wBuf[11], &bBuf[11],
                INPUT_DIM[11], OUTPUT_DIM[11], NBYN[11], current_batch_size,
                &conv_event[7], &conv_event[8]);

            convolution_layer(queuelist[13], conv_kernel, &layerBuf[11], &layerBuf[12], &wBuf[12], &bBuf[12],
                INPUT_DIM[12], OUTPUT_DIM[12], NBYN[12], current_batch_size,
                &conv_event[8], &conv_event[9]);

            max_pooling_layer(queuelist[14], pooling_kernel, &layerBuf[12], &layerBuf[13],
                INPUT_DIM[13], NBYN[13], current_batch_size,
                &conv_event[9], &pool_event[3]);



            convolution_layer(queuelist[15], conv_kernel, &layerBuf[13], &layerBuf[14], &wBuf[14], &bBuf[14],
                INPUT_DIM[14], OUTPUT_DIM[14], NBYN[14], current_batch_size,
                &pool_event[3], &conv_event[10]);

            convolution_layer(queuelist[16], conv_kernel, &layerBuf[14], &layerBuf[15], &wBuf[15], &bBuf[15],
                INPUT_DIM[15], OUTPUT_DIM[15], NBYN[15], current_batch_size,
                &conv_event[10], &conv_event[11]);

            convolution_layer(queuelist[17], conv_kernel, &layerBuf[15], &layerBuf[16], &wBuf[16], &bBuf[16],
                INPUT_DIM[16], OUTPUT_DIM[16], NBYN[16], current_batch_size,
                &conv_event[11], &conv_event[12]);

            max_pooling_layer(queuelist[18], pooling_kernel, &layerBuf[16], &layerBuf[17],
                INPUT_DIM[17], NBYN[17], current_batch_size,
                &conv_event[12], &pool_event[4]);


            fully_connected_layer(queuelist[19], fc_kernel, &layerBuf[17], &layerBuf[18], &wBuf[18], &bBuf[18],
                INPUT_DIM[18], OUTPUT_DIM[18], current_batch_size,
                &pool_event[4], &conv_event[13]);

            fully_connected_layer(queuelist[20], fc_kernel, &layerBuf[18], &layerBuf[19], &wBuf[19], &bBuf[19],
                INPUT_DIM[19], OUTPUT_DIM[19], current_batch_size,
                &conv_event[13], &conv_event[14]);

            fully_connected_layer(queuelist[21], fc_kernel, &layerBuf[19], &layerBuf[20], &wBuf[20], &bBuf[20],
            ...
            save_layer(queuelist[22], save_kernel, &layerBuf[20], &labels[i], &confidences[i], current_batch_size, &conv_event[15], &save_event);
