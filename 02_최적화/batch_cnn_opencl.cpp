...
#define CHECK_ERROR(err) \
        ...
        int dim, int nbyn, int batch_size)
    {
        ...
        size_t global_work_size[4] = { nbyn, nbyn, dim, batch_size };
        err = clEnqueueNDRangeKernel(queue, kernel, 4, NULL, global_work_size, NULL, 0, NULL, NULL);
        CHECK_ERROR(err);
    }
...
void cnn_batch(float* images, float* network, int* labels, float* confidences, int num_of_image, int batch_size) {
    cl_int err;
    ...
    cl_mem layerBuf[21];
    for (int i = 0; i < 21; ++i) {
        layerBuf[i] = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * OUTPUT_DIM[i] * NBYN[i] * NBYN[i] * batch_size, NULL, &err);
        CHECK_ERROR(err);
    }
