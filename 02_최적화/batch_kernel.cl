...
__kernel void conv_kernel(
    ...
    outputs[batch_idx * outNeuron * nbyn * nbyn + (outNeuron * nbyn * nbyn) + (row * nbyn) + col] = fmax(sum + biases[outNeuron], 0.0f);
}
...
__kernel void pooling_kernel(
    ...
    const int batch_idx = get_global_id(3);
    ...
    float maxVal = inputs[batch_idx * n * inputNbyn * inputNbyn + (n * inputNbyn * inputNbyn) + (inputRow * inputNbyn) + inputCol];
    maxVal = fmax(inputs[batch_idx * n * inputNbyn * inputNbyn + (n * inputNbyn * inputNbyn) + (inputRow * inputNbyn) + inputCol + 1], maxVal);
    maxVal = fmax(inputs[batch_idx * n * inputNbyn * inputNbyn + (n * inputNbyn * inputNbyn) + ((inputRow + 1) * inputNbyn) + inputCol], maxVal);
    maxVal = fmax(inputs[batch_idx * n * inputNbyn * inputNbyn + (n * inputNbyn * inputNbyn) + ((inputRow + 1) * inputNbyn) + inputCol + 1], maxVal);

    outputs[batch_idx * n * nbyn * nbyn + (n * nbyn * nbyn) + (row * nbyn) + col] = maxVal;
}
...
__kernel void fc_kernel(
    ...
    outputs[batch_idx * outNeuron + outNeuron] = fmax(sum + biases[outNeuron], 0.0f);
}
